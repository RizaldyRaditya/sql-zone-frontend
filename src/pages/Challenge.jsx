...
const CHALLENGE_DURATION = 300; // seconds per challenge
...
const Challenge = () => {
  ...
  // FIX 1: Initialize timeLeft to CHALLENGE_DURATION (was 180, inconsistent with reset value of 300)
  ...
  const [worldChallengeStatus, setWorldChallengeStatus] = useState({});
  const [penaltyAnimation, setPenaltyAnimation] = useState(false);
  const [schema, setSchema] = useState([]);

  // FIX 2: Removed initialTimeLeft state — timeUsed is now tracked directly via a ref
  // to avoid the bug where initialTimeLeft - timeLeft gave negative values after reload
  const timeUsedRef = useRef(0);

  // FIX 3: Changed from useRef to useState so useEffect timer re-runs when timerActive changes
  ...
  useEffect(() => {
    ...
    if (userStr) {
      try {
        ...
        // FIX 4: Guard against corrupted localStorage data
  ...
  const setCurrentChallengeData = (updates) => {
    setChallengeData((prev) => ({
      ...
      [selectedChallengeSequence]: {
        // FIX 5: Use functional prev to avoid stale closure bug in setCurrentChallengeData
        ...(prev[selectedChallengeSequence] || {
          userQuery: "",
          queryResult: null,
          queryError: "",
          submissionResult: null,
    ...
    try {
      ...
      if (userQueryResponse.data.success) {
        ...
        if (expectedQueryResponse.data.success) {
          ...
          // FIX 6: timeUsed is now tracked directly from the ref, not from initialTimeLeft - timeLeft
          const timeUsedValue = timeUsedRef.current;

          if (isCorrect) {
            const calculatedScore =
              selectedChallenge.baseScore +
              Math.max((CHALLENGE_DURATION - timeUsedValue) * 2);
            ...
            // FIX 7: Stop the timer once challenge is answered correctly
            ...
            // Penalti waktu 30 detik untuk jawaban salah
      ...
      if (errorMessage.includes("syntax error")) {
        ...
        errorMessage.toLowerCase().includes("table") && errorMessage.toLowerCase().includes("exist")
      ) {
        hint =
  ...
  useEffect(() => {
    ...
    if (completedCount === challenges.length && completedCount > 0) {
      ...
      if (user) {
        api.user
          .updateProgress({ userId: user.id, worldId, totalScore, totalTime })
          .then((response) => {
            const newStatusMap = {};
  ...
  // FIX 8: Timer — only fires "WAKTU HABIS" when activeTab === "level" AND timerActive is true
  // timerActive is now useState (not useRef) so useEffect properly re-runs when it changes
  useEffect(() => {
    let timer;

    if (activeTab === "level" && timerActive && timeLeft > 0) {
      timer = setInterval(() => {
        setTimeLeft((prev) => {
          // FIX 9: Track timeUsed directly via ref each tick — no more initialTimeLeft math
      ...
      // FIX 10: Only alert when timer was genuinely running (timerActive === true)
      setTimerActive(false);
  ...
  }, [activeTab, timeLeft, timerActive, worldId, selectedChallengeSequence]);

  // FIX 11: Timer init — properly handles reload mid-challenge
  // Expired localStorage keys are cleaned up instead of showing timeLeft = 0
  useEffect(() => {
    ...
    if (savedStartTime) {
      ...
      // FIX 12: Validate that savedStartTime is a real timestamp, not corrupted data
      ...
      const remaining = CHALLENGE_DURATION - elapsedSeconds;

      if (remaining <= 0) {
        // FIX 13: If timer already expired when reopening, reset cleanly instead of showing 0 → alert instantly
        localStorage.removeItem(timerKey);
        localStorage.setItem(timerKey, Date.now().toString());
        timeUsedRef.current = 0;
        setTimeLeft(CHALLENGE_DURATION);
        ...
        // FIX 14: timeUsedRef reflects elapsed time so score calculation stays correct on reload
        timeUsedRef.current = elapsedSeconds;
              ...
              <div className="space-y-4">
                <button
                  onClick={() => {
                    ...
                    // FIX 15: Pause timer when switching away from level tab
                    setTimerActive(false);
                  }}
              ...
              <div className="animate-fade-in overflow-y-auto max-h-[calc(100vh-240px)] pr-4">
                ...
                {materiLoading ? (
                  ...
                  <div className="bg-red-500/10 border border-red-500 rounded-lg p-6 text-center">
                    ...
                    {/* FIX 16: Use production URL instead of hardcoded localhost */}
                    <a
                      href={`${import.meta.env.VITE_API_URL || ""}${materi?.materiContent}`}
                      target="_blank"
              ...
              <div className="space-y-6 mb-8">
                ...
                <div className="bg-blue-500/20 border border-blue-500 rounded-xl p-4">
                  ...
                  <p className="text-blue-400 text-3xl font-bold">
                    {(() => {
                      // FIX 17: Use timeUsed from challengeData (always positive, tracked via ref)
                      const totalSeconds = Object.values(challengeData).reduce(
                        (total, data) => {
                          return total + Math.max(0, data.timeUsed || 0);
                        },
                        0,
                      );
