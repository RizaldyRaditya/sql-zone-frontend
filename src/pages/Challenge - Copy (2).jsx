...
const Challenge = () => {
  ...
  // FIX 3: Guard ref to prevent "WAKTU HABIS" from firing before a challenge is active
  ...
  // FIX 8: Timer — only fires "WAKTU HABIS" when activeTab === "level" AND timerActiveRef is true
  // This prevents false alerts on mount, tab switch, or stale state
  useEffect(() => {
    let timer;

    if (activeTab === "level" && timerActiveRef.current && timeLeft > 0) {
      timer = setInterval(() => {
        setTimeLeft((prev) => {
      ...
      // FIX 10: Only alert when timer was genuinely running (timerActiveRef.current === true)
      timerActiveRef.current = false;
